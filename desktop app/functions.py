...
def extract_features_audio_file(file, offset=0, duration=None):
...
def classify_music_folder(music_files, out_directory, classifier="svm"):
    model = pickle.load(open("models/" + classifier + "_model_music", 'rb'))
    for f in music_files:
        filename = os.path.basename(f)
        if os.path.isfile(f) and (f.endswith(".wav") or f.endswith(".mp3")):
            features = extract_features_audio_file(f, offset=30, duration=60)
            test_frame = normilize_newdata(features)
            pred = model.predict_proba(test_frame)[0]
            genre_pred = decode_label([pred.argmax()])
            pred_path = os.path.join(out_directory, genre_pred)
            if not os.path.exists(pred_path):
                os.makedirs(pred_path)
            shutil.copy(f, os.path.join(pred_path, filename))